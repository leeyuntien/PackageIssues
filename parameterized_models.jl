...
function cumhazard(m::AlterMakeham,age) 
    @unpack μ, σ, c = m
    return exp(-μ / σ) .* (exp.(age ./ σ) .- 1) .+ age .* c
end
...
function hazard(m::InverseGompertz,age)
    @unpack μ, σ = m 
    return 1 / σ .* exp.(-(age .- μ) ./ σ) ./ (exp.(exp.(-(age .- μ) ./ σ)) .- 1)
end
...
function survival(m::InverseGompertz,age) 
    @unpack μ, σ = m 
    return (1 .- exp.(-exp.(-(age .- μ) ./ σ))) ./ (1 - exp(-exp(μ / σ)))
end
